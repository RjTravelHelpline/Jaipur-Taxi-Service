...
const TourEnquiryForm = ({ selectedPackage }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(
                "http://localhost:5000/api/tour-enquiry",
                {
                    method: "POST",
                    headers: {
    ...
    const vehicleTypeOptions = CarRentalsByCategory.map((item) => item.title);
    const vehicleNameOptions = CarRentalsByCategory.reduce((acc, item) => {
        acc[item.title] = item.vehicles;
        return acc;
    }, {});
    ...
    return (
        <div className="container contact-container px-0">
            <div className="row d-flex flex-column justify-content-center align-items-center w-100">
                <div className="d-flex flex-column gap-2 justify-content-center p-0 contact-popup">
                    {isSubmitted ? (
                        ...
                        >
                            <div className="w-100 py-4">
                                <h3 className="rounded-2 text-capitalize mb-0 p-4 package-form web-title">
                                    {selectedPackage ? (
                                        <>
                                            <span className="fw-bold">Tour Enquiry for </span>
                            ...
                            <div className="row p-4">
                                ...
                                <div className="col-12 col-lg-6 col-md-6 my-2">
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className="w-100"
                                        disabled={!formData.vehicleType} // Disable until vehicleType is selected
                                    >
                                        <option value="" disabled>
                                            Select vehicle name
                                        </option>
                                        {formData.vehicleType &&
                                            vehicleNameOptions[formData.vehicleType].map((name) => (
